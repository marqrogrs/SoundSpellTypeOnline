...
//July 18, 2020
//Some word data was cleaned in data.js, but not in SoundSpellerDatabase.json (the legacy database file — filename kept for compatibility)
...
uncleanedWords = uncleanedWords.map((uncleanedWordObj) => {
  //For each uncleaned word object...
  var cleanedObj;
  cleanedWords.forEach((cleanedWordArr) => {
    //Iterate over all the cleaned word arrays
    if (cleanedWordArr[0] === parseInt(uncleanedWordObj.word_id)) {
      //and if the IDs are equal...
